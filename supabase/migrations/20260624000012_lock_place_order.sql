-- ============================================================================
-- Lock down order placement so it can ONLY happen via the server endpoint
-- (which verifies a Cloudflare Turnstile token first). This makes the captcha
-- impossible to bypass via the public anon key.
--
-- place_order now takes an explicit p_user_id (the server passes the verified
-- session user) and is callable ONLY by service_role.
-- ============================================================================

drop function if exists public.place_order(jsonb, text, text, text, text, text, timestamptz);
...
-- Only the server (service_role) may place orders now.
revoke execute on function
  public.place_order(jsonb, text, text, text, text, text, timestamptz, uuid)
